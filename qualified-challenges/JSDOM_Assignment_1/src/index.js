...
function filterByCity(city) {
  const newFilter = contacts.filter((contact) => contact.address.city === city);
...
function filterHandler() {
  const filterHandler = document.querySelector("#filterOptions");

  filterHandler.addEventListener("change", (event) => {
    if (event.target.value === 0) {
      render(contacts);
    } else {
      const filterList = filterByCity(event.target.value);
      render(filterList);
    }
  });
}
...
function loadCities(contacts) {
  // get all the city in the contacts
  // add it to the selct option
  // get the select
  // add new option to it
  //contacts = [];

  //   if (contacts.length > 0) {
  //     const cities = contacts.map((contact) => {
  //       return contact.address.city;
  //     });

  //     //console.log(cities);

  //     const uniqueCity = cities.filter(onlyUnique);
  ...
  //     uniqueCity.forEach((cityElement) => {
  //       const option = document.createElement("option");

  //       option.value = cityElement;

  //       option.innerText = cityElement;
...
function deleteContact(id) {
  // const deleteContact = document.querySelector(`#data-id=${id}`);

  // const section = document.querySelector("#contacts");

  // if (deleteContact) section.removeChild(deleteContact);
...
function deleteButtonHandler() {
  ...
  //   deleteContact(event.target.value.id);
